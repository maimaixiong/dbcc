...
static int signal2print(signal_t *sig, unsigned id, const char *msg_name, FILE *o)
{
	...
	/*super lazy*/
	if (sig->is_floating)
		return fprintf(o, "\tr = print_helper(r, chprintf(output, \"%s = (wire: %%g)\\n\", (double)(o->%s.%s)));\n", sig->name, msg_name, sig->name);
	return fprintf(o, "\tr = print_helper(r, chprintf(output, \"%s = (wire: %%u)\\r\\n\", (o->%s.%s)));\n", sig->name, msg_name, sig->name);
}
...
static int msg_print(can_msg_t *msg, FILE *c, const char *name, const char *god, dbc2c_options_t *copts)
{
	...
	assert(god);
	assert(copts);
	fprintf(c, "int print_%s(const can_obj_%s_t *o, BaseSequentialStream *output) {\n", name, god);
	if (copts->generate_asserts) {
		fputs("\tassert(o);\n", c);
		fputs("\tassert(output);\n", c);
...
static int switch_function_print(FILE *c, dbc_t *dbc, bool prototype, const char *god, dbc2c_options_t *copts)
{
	...
	assert(god);
	assert(copts);
	fprintf(c, "int print_message(const can_obj_%s_t *o, const unsigned long id, BaseSequentialStream *output)", god);
	if (prototype)
		return fprintf(c, ";\n");
	fprintf(c, " {\n");
	if (copts->generate_asserts) {
...
int dbc2c(dbc_t *dbc, FILE *c, FILE *h, const char *name, dbc2c_options_t *copts)
{
	...
	/* C FILE */
	fputs("/* Generated by DBCC, see <https://github.com/maimaixiong/dbcc> */\n", c);
	fprintf(c, "#include \"%s\"\n", name);
	...
	fputc('\n', c);
    fprintf(c, "#include \"ch.h\"\n");
    fprintf(c, "#include \"hal.h\"\n");
    fprintf(c, "#include \"chprintf.h\"\n\n");
